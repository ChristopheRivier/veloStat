...
Application **Next.js 16** + **Supabase** (auth, base PostgreSQL avec RLS). Analyse réalisée sur le code source, le schéma SQL, le middleware et les dépendances.
...
### 2.1 Callback auth — redirection

**Fichier :** `src/app/auth/callback/route.ts`

Le paramètre `next` est validé via une whitelist `ALLOWED_REDIRECT_PATHS` et une fonction `getSafeRedirectPath()` qui rejette les chemins non internes (ex. `//evil.com`), normalise les trailing slashes et n’accepte que `/`, `/stats`, `/dashboard`, `/bikes`, `/login` et leurs sous-chemins.

---

### 2.2 Dépendances

**Next.js** : version **16.1.6** en usage ; les CVE concernant la branche 14.x (dont GHSA-f82v-jwr5-mffw) ne s’appliquent pas à cette version. Overrides pour `minimatch` et `glob` en place pour limiter les risques ReDoS côté outillage. Veille régulière avec `npm audit` recommandée.
...
| **P1** | Exécuter `npm audit` régulièrement et appliquer les correctifs. |
...
- **Next.js** : version **16.1.6** en usage ; les vulnérabilités critiques signalées sur la branche 14.x ne s’appliquent pas à cette version.
- **Overrides** : `minimatch` et `glob` sur des versions récentes pour limiter les ReDoS (outillage dev).
- Veille régulière : `npm audit` et `npm audit fix`.
...
*État des lieux du projet. Revoir après chaque changement de stack ou de schéma.*
